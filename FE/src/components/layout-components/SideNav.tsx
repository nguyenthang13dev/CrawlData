...
export const SideNav = () => {
  ...
  const themeConfig = useSelector((state: RootState) => state.theme.theme); // Lấy themeConfig từ Redux
  const onToggle = () => {
    if (!isMobile) {
  ...
  return (
    ...
    >
      <Scrollbars autoHide>
        <div className="p-2">
          <div
            className="flex gap-1 align-items-center hover text-white p-[10px] rounded-sm cursor-pointer toggle-button"
            style={{
              backgroundColor: themeConfig?.token?.colorPrimary || "#CE1127",
            }}
            onClick={onToggle}
          >
            <MenuOutlined />
            {!navCollapsed && (
              <>
                ...
                <div>
                  <ArrowLeftOutlined className="text-yellow-200" />
                </div>
              </>
            )}
          </div>
