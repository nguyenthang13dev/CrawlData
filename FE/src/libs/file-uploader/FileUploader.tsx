...
function useFileUploader(
  ...
  const [files, setFiles] = useState<TaiLieuDinhKem[]>(initFiles);
  ...
  const handleRemove = async (file: UploadFile) => {
    Modal.confirm({
      ...
      onOk: async () => {
        // Xoá khỏi state
        setFiles((prev: TaiLieuDinhKem[]) =>
          prev.filter((f: TaiLieuDinhKem) => f.duongDanFile !== file.url)
        );
  ...
  const customRequest: UploadProps["customRequest"] = async (options) => {
    ...
    try {
      ...
      if (
        ...
        setFiles((prev: TaiLieuDinhKem[]) => [...prev, ...result.data]);
        onSuccess?.(result);
      } else {
